...
window.GYMSPIRE_TEMPLATES.push({
  ...
  fieldsForSlide(index) {
    if (index === 0) return ['bgImage', 'tip1', 'tip2', 'tip3', 'tagline'];
  ...
  render(s, slideIndex, total) {
    ...
    const tipHtml = tips.map((tip, i) => `
      <div style="display:flex;align-items:flex-start;gap:20px;margin-bottom:52px;">
        <span style="font-family:${f.h};font-size:48px;font-weight:${f.hW};color:#2B9BF4;line-height:1;min-width:56px;">0${i + 1}</span>
        <span style="font-family:${f.h};font-size:36px;font-weight:${f.hW};color:#fff;line-height:1.3;">${tip}</span>
      </div>
    `).join('');
    return `
      <div style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,0.72) 0%,rgba(0,0,0,0.88) 100%);"></div>
      <div style="position:absolute;top:48px;left:0;right:0;text-align:center;font-family:${f.h};font-size:22px;font-weight:${f.hW};letter-spacing:4px;color:rgba(255,255,255,0.85);">GYMSPIRE</div>
      <div style="position:absolute;top:130px;left:48px;right:48px;">${tipHtml}</div>
      <div style="position:absolute;bottom:80px;left:48px;font-family:${f.b};font-size:24px;font-weight:${f.bW};color:rgba(255,255,255,0.55);letter-spacing:2px;">${s.tagline || ''}</div>
    `;
  }
});
