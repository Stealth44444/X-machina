  ...
  render(s, slideIndex, total) {
    ...
    const tipHtml = tips.map((tip, i) => `
      <div style="display:flex;align-items:flex-start;gap:20px;margin-bottom:52px;">
        ...
        <span style="font-size:36px;font-weight:700;color:#fff;line-height:1.3;">${tip}</span>
      </div>
    `).join('');
    return `
      ...
      <div style="position:absolute;bottom:80px;left:48px;font-size:24px;font-weight:400;color:rgba(255,255,255,0.55);letter-spacing:2px;">${s.tagline || ''}</div>
    `;
  }
});
